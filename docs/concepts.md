...
drives a live session) — that rides on the parked substrate research ([EVENT-ROUTER](architecture/event-router.md)).
...
| **Mail** | The typed, persisted bus — the *only* agent↔agent channel. **Actionable** mail stays unread until acted on; **informational** does not. Rendered per audience: structured for agents, a real inbox for the operator (Principle 3 — render-per-audience, [MAIL-BUS](architecture/mail-bus.md)). |
...
provider headroom refreshes ([DISPATCH](architecture/dispatch.md)). The exact state set may refine once the session model lands —
