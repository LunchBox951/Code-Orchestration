...
mail, ran one turn, and slept — is **superseded by the Conductor** ([CORE-CONCEPTS](../concepts.md)), the named
...
block ([PERMISSIONS](permissions.md)).
...
- **Spawn & transport** — how a placed agent is launched and hosted (Codex `exec` vs `app_server`,
  the equivalent Claude path) — [DISPATCH](dispatch.md), [PROVIDERS](providers.md).
- **Liveness** — telling a dead/zombie session from a slow turn (the live watchdog) —
  [HEALTH-and-DIAGNOSTICS](health-and-diagnostics.md), [STATE-and-RECOVERY](state-and-recovery.md).
...
Full open-question set: [runtime-substrate](../research/runtime-substrate.md).
