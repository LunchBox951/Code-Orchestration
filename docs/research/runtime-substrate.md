...
The runtime engine that drives these sessions is the **Conductor** ([CORE-CONCEPTS](../concepts.md),
[EVENT-ROUTER](../architecture/event-router.md)) — named and role-fixed, mechanism-deferred.

## Open questions (substrate-dependent)

Harvested from every topic that flagged a deferral:

1. **Turn execution** ([CORE-CONCEPTS](../concepts.md), [EVENT-ROUTER](../architecture/event-router.md)) — how does the Conductor run *one turn* of a
...
2. **Spawn & transport** ([DISPATCH](../architecture/dispatch.md), [PROVIDERS](../architecture/providers.md)) — how is a placed agent actually launched and hosted?
...
3. **Live-session recovery** ([STATE-and-RECOVERY](../architecture/state-and-recovery.md)) — can a provider session `--resume`? How much
...
4. **Liveness / watchdog** ([HEALTH-and-DIAGNOSTICS](../architecture/health-and-diagnostics.md), [PERMISSIONS](../architecture/permissions.md)) — how to distinguish a truly dead or
...
5. **Block-enforcement layer** ([PERMISSIONS](../architecture/permissions.md)) — are the hard blocks enforced via the host harness'
...
6. **Agent-console reliability** ([TUI](../architecture/tui.md)) — making the live terminal-emulator pane reliable:
