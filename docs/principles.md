...
| 7 | `gated-by-default` | Nothing reaches master/remote/PR without a **PASS** (agent *or* human). Two verdicts; the blocker bar **tightens toward production**. Ruthless only *because* escalation gives an exit. | [REVIEW-GATES](architecture/review-gates.md) · [AGENT-ROLES](architecture/agent-roles.md) · [WORKTREES](architecture/worktrees.md) |
