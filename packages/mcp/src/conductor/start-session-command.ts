 ...
 *
 * `co-mcp start-session <projectId> --prompt "…"`  OR  `co-mcp start-session <projectId> --spec <path>`
 *
...
import { openRegistry, startCoordinatorSession, type ProjectRegistry } from '@co/core';
...
/**
 * Parse `start-session <projectId> (--prompt "…" | --spec <path>)`. Fails loud (Principle 9) on a
  ...
  if (projectId == null || projectId.trim().length === 0 || projectId.startsWith('--')) {
    throw new Error(
      'co-mcp start-session: a project id is required ' +
        '(usage: `co-mcp start-session <projectId> --prompt "…" | --spec <path>`).',
    );
  }
...
export async function runStartSessionCommand(
  ...
  const { projectId, prompt, specPath } = parseStartSessionArgs(argv);
