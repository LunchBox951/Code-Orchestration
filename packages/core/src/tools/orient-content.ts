...
const COORDINATOR_GUIDANCE = [
  ...
  'dispatch a lead per phase, gate each returned result, then publish and close.',
  '',
...
const LEAD_GUIDANCE = [
  ...
  'your coordinator.',
  '',
  'Dispatch sequentially — only once a worker’s branch has been reviewed and merged do you hand out',
  'the next task — so integration stays clean. You do not finish through the gate yourself; you',
  'stitch together the reviewed branches your workers produce.',
  '',
...
const IMPLEMENTER_GUIDANCE = [
  'As an implementer you change code in your own isolated worktree, then finish through the gate.',
  'Read your inbox, make the focused change as small reviewable commits, and verify it with the',
  'project’s own test and check commands before you hand it back.',
  '',
  'Finish through the gate: commit, then hand off to the finisher, which dispatches a reviewer. When',
  'the review passes, your done signal goes to your parent — integrating and publishing the branch',
  'is your parent’s job, never yours.',
  '',
