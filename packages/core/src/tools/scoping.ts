...
/**
 * SEED per-role toolsets over the CURRENT ten `co_*` tools — the mechanism plus a defensible
 * starting membership, NOT the authoritative rosters. The concrete, authoritative per-role rosters
 * (and the later gated tools they will gain — `co_finish`, `co_merge`, … — which are L5/L6) are not
 * locked here; this seed exists to prove the per-role scoping hook works today (AC-L2-5).
