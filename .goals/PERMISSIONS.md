...
Each role has a declarative **permission profile** governing what tool calls and
shell commands it may run.

- Permissions are defined in a single registry and reconciled against the live
  runtime.
- **`co permissions regen`** — regenerate the registry from source.
- **`co permissions check`** — compare the declared registry against the live
  runtime and fail on drift.
- Hard denials apply to *all* roles: no `git push --force` / `-f` /
  `--force-with-lease`, no `rm -rf /` or `rm -rf ~`, no `sudo`, no invoking the
  daemon `co run` from a tool call.
- The **Reviewer** has full write permission at the permission level; its
  *read-only-for-code* discipline is enforced by the prompt, not the permission
  layer (so it can stamp verification markers).

### Gate hooks (defense in depth)

`co init` installs hooks that block dangerous/ungated actions at the harness level:

- **Publishing-verb gate** — refuses raw `git merge` / `git push` / `gh pr merge`,
  forcing the gated `co` verbs.
- **Polling-loop gate** — blocks agents from busy-waiting / polling loops.

Both ship in Claude and Codex variants.
...
- **Agents invoking `co` in the shell** → forces the MCP surface; enforces the
  no-fallback / single-surface decision (see MCP-TOOLS).
...
- **Sub-role focus is prompt-shaped, not hard-cut.** `implementer:docs` is *nudged* to
  ...
  These are approach specializations (AGENT-ROLES) guided by prompt — not a permission
...
- **Reviewer stays out of the code it reviews — by prompt, not a wall.** It reads, runs
  ...
  lapses were *leniency* (the strict-gate posture — REVIEW-GATES), not editing.
