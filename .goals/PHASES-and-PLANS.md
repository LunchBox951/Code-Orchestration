...
Larger tasks are decomposed into **phases**, each owned by one Lead and
independently mergeable.

### Plans

- **`co plan validate`** — validate a plan file (no DB writes), surfacing soft
  validator reminders.
- **`co plan ingest`** — validate then upsert phases into the state DB.
- **`co plan show`** — print plan frontmatter (or a single phase) as JSON.
- **`co plan list`** — list phases known to the DB for a task.

A Planner writes `<repo-root>/.co/plans/<task-id>.md` with Overview, Phases,
Dependencies (must form a DAG), and acceptance checks citing the project's real
verification command.

### Phase management & gating

- Phase verbs (`add` / `list` / `show` / `set-status` / `set-lead` / `addendum` /
  `close`) manage the phase lifecycle.
- **`co phase-status`** — the structured snapshot used by the `phase_chunk_ready`
  gate. For a Lead it returns that Lead's phase snapshot; for a Coordinator, one
  snapshot per owned phase.
- **`co coord-phase`** — set a Coordinator's phase-modal overlay
  (`brainstorm` → `build` → `publish`), used after spec-lock and after the final
  PR opens.
- **Phase testing** — after all worker merges land, a phase tester runs the suite
  against the lead branch and reports counts; the Lead then mails `phase_ready`.
...
for the strict reviewer (REVIEW-GATES), not a judgment call.
...
1. **Mechanically provable.** Each criterion maps to a test; **phase-done = its criteria-tests
   pass AND no regression vs the baseline** (REVIEW-GATES). The reviewer's "criteria unmet" is
...
2. **The plan validator rejects fuzzy criteria.** A plan can't be ingested if a criterion
   ...
   hoped for — the completeness discipline (MCP-TOOLS) applied to plans.
...
- **Max active children per parent** *(configurable setting; excludes reviewers).* A
  ...
  frees. The primary throttle on fan-out (and on rate burn — see DISPATCH/COST for the
...
> Per-role model preferences and rate-limit-aware provider routing live in DISPATCH / COST;
