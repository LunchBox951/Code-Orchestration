...
A **cosmetic companion** — a piece of ASCII-art personality that lives in the TUI.

- Purely decorative; **never gates real agent work**.
- **`co buddy show / roll / reroll / history`** — view your current buddy, roll a
  new one, reroll (limited to once per day), or browse archived buddies.
- A catalog of ASCII-art **faces** organized by tier/category/creature; face
  dimensions are computed from the art, not hand-typed.
- Buddy "brain" copy is **Anthropic-backed when credentials exist**, else degrades
  to static fallback text. Codex-only projects can force `tui.buddy.backend:
  fallback` to avoid any Anthropic/Claude probe.
...
throttled first under rate pressure (DISPATCH / COST). Provider-backed when credentials exist,
