...
- The **locked spec** is the source of truth for a task; the Coordinator writes it
  under `<repo-root>/.co/specs/` and locks it (operator-only `/lock` in the TUI).
- **Spec lifecycle policy**: specs are *deleted* once their work merges — git
  history is the durable record. Exceptions: `00-`-prefixed standing orders
  survive until their condition is met; rolling support artifacts are edited in
  place, not deleted.

### Issues

- **`co issue new` / `co issue list`** — create and list `.co/issues/` entries,
  lightweight tracked problems/observations.

### Decision Ledger

- **`co ledger show` / `add` / `path`** — a per-task decision ledger. Agents
  append bullets to named sections to record decisions and rationale durably
  across turns.
...
the Vision's two asymmetric surfaces.)
...
2. **Diagnose.** It dispatches a **`researcher:diagnostic`** (see AGENT-ROLES) that
...
> WORKTREES. The `researcher:diagnostic` sub-role is recorded in AGENT-ROLES.
