...
(Principle 4 — one-agent-surface; [`../.goals/MCP-TOOLS.md`](../.goals/MCP-TOOLS.md)) inside a
...
- Three background agents were each asked the same question — *"what language should we use?"* —
  ...
  Rust, one could not pick Go. Each read `PORTING-CO.md` and reasoned from the `.goals/` corpus
  independently.
...
(Principle 2 — authentic-terminal; [`../.goals/TUI.md`](../.goals/TUI.md),
[`../.goals/EVENT-ROUTER.md`](../.goals/EVENT-ROUTER.md), and `runtime-substrate.md` Q1/Q6).
...
3. **"One core, thin adapters" with zero serialization seams.** The Electron main process, the MCP
   ...
   ([`../.goals/MCP-TOOLS.md`](../.goals/MCP-TOOLS.md)).
4. **Node's async event loop fits the Conductor.** Supervising many concurrent pty sessions, the
   ...
   ([`../.goals/DISPATCH.md`](../.goals/DISPATCH.md),
   [`../.goals/COST-and-USAGE.md`](../.goals/COST-and-USAGE.md)) is pure I/O multiplexing — Node's
...
5. **Typed envelopes are cheap and safe.** The mail bus, MCP schemas, specs, and event-sourced
   records ([`../.goals/MAIL-BUS.md`](../.goals/MAIL-BUS.md),
   [`../.goals/STATE-and-RECOVERY.md`](../.goals/STATE-and-RECOVERY.md)) are JSON contracts; one
...
- **Native-addon packaging** — `node-pty` / `better-sqlite3` need per-platform prebuilds across
  Linux/macOS/Windows ([`../.goals/WORKTREES.md`](../.goals/WORKTREES.md),
  [`../.goals/INIT-and-CONFIG.md`](../.goals/INIT-and-CONFIG.md) auto-update).
